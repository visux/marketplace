...
describe("Marketplace", function () {
  it("Should create and sell NFTS", async function () {
    ...
    const SonikchainCollectionNFT = await ethers.getContractFactory("SonikchainCollection");
    const nft = await SonikchainCollectionNFT.deploy(marketAddress);
    await nft.deployed();
    ...
    const nft1 = await nft.createSonikchain("QmR9iasTahv1HZRP4DsZySxn1m3qzvBKXsGhkKyWcfFuHV"); // Hera Wink
    await nft1.wait();
    const nft2 = await nft.createSonikchain("QmYqb3cCkRQazJPqp29BoQGVP118ikA7e1hEsWSB4pZmnk"); // Hera Explore
    await nft2.wait();
