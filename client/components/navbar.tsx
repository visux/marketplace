...
const navigation = [
  { name: "Metaverse", href: "/" },
  { name: "My buyed NFTs", href: "/myNFTs" },
  { name: "Create your NFT", href: "/createItem" },
];
  ...
  return (
    <Disclosure
      ...
      className={clsx("fixed z-40 w-full transition-all ease-in-out duration-200 py-4", {
        " bg-[#02011a] shadow-[#02011a]/20 shadow-md py-1": scrolled,
      })}
    >
      {({ open }) => (
        <>
          <div className="px-2 mx-auto max-w-7xl sm:px-6 lg:px-8">
            <div className="relative flex items-center justify-between h-16">
              <div className="absolute inset-y-0 left-0 flex items-center sm:hidden">
                {/* Mobile menu button*/}
                <Disclosure.Button className="inline-flex items-center justify-center p-2 text-gray-400 rounded-md hover:text-white hover:bluecol focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white">
              ...
              <div className="absolute inset-y-0 right-0 flex items-center pr-2 space-x-4 font-medium sm:static sm:inset-auto sm:ml-6 sm:pr-0">
                ...
                {connectedAccount ? (
                  <Menu as="div" className="relative ml-3">
                    <div>
                      <Menu.Button className="flex items-center max-w-xs px-4 py-2 text-white transition rounded-full bg-gradient-to-tl from-primary via-purple-500 to-defaultcol hover:bg-bluecol shadow-homogen font-poppins">
                    ...
                    >
                      <Menu.Items className="absolute right-0 py-1 mt-2 origin-top-right bg-bluecol rounded-md shadow-lg w-36 ring-1 ring-black ring-opacity-5 focus:outline-none">
                        <Menu.Item>
                          <div className="m-2 rounded-md hover:bg-bluecol">
          ...
          <Disclosure.Panel className="bg-gray-800 sm:hidden">
            <div className="px-2 pt-2 pb-3 space-y-1">
              <>
                {navigation.map((item) => (
                  <Link key={item.name} href={item.href}>
                    <div className="block px-3 py-2 text-base font-medium text-gray-300 rounded-md cursor-pointer hover:bg-bluecol hover:text-white">
                      {item.name}
                    </div>
                  </Link>
                ))}
